...
clear;
clc;
data_path = fullfile(pwd,filesep, "dataset");
addpath(data_path);
code_path = fullfile(pwd,filesep, "my_tools");
addpath(code_path);
...
for i1 = 1 : length(datasetCandi)
    ...
    fid = fopen([pwd,'/MBPC_data_result/',data_name,'_Result.txt'],'a'); % build the txt file to save clustering results
    tic;
    %% ==================Training TBGL================%%
    ...
    %% ======================Record=====================%%
    fprintf(fid,'lambda:%f\t gama12:%f\t lp:%f\t ',alpha,gama,p);
    fprintf(fid,'ACC:%g\t NMI:%g\t Purity:%g\t Precision:%g\t Recall:%g\t Fscore:%g\t ARI:%g  time:%g\n ',Clu_result',time);
    fclose(fid);
end