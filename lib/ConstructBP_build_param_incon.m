function paramCell = ConstructBP_build_param(anchor_meta_type, anchor_types,anchor_sizes_cell, weight_types, knn_sizes, diffusion_params, betas)

if ~exist('anchor_types', 'var')
    anchor_types = {'km', 'km+', 'random'};
end

if ~exist('anchor_sizes_cell', 'var')
    anchor_sizes = 100;
end
...
if strcmpi(anchor_meta_type, 'hier') || strcmpi(anchor_meta_type, 'plain')
    nParam = length(anchor_types) * length(anchor_sizes_cell) * length(weight_types) * length(knn_sizes) * length(diffusion_params);
    paramCell = cell(nParam, 1);
    idx = 0;
    for i1 = 1:length(anchor_types)
        for i2 = 1:length(anchor_sizes_cell)
            for i3 = 1:length(weight_types)
                for i4 = 1:length(knn_sizes)
                    for i5 = 1:length(diffusion_params)
                        ...
                        param.nAnchor = anchor_sizes_cell{1,i2};
